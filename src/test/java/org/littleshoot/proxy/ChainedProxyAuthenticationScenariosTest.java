 ...
 *
 * <p>1. LittleProxy handles auth, next proxy does not 2. Both LittleProxy and next proxy handle
...
class ChainedProxyAuthenticationScenariosTest {
   ...
   *
   * <p>This is the most complex case requiring two roundtrips: 1. Client → [Proxy-Authorization:
   * clientCreds] → LittleProxy (authenticates) 2. LittleProxy → [Proxy-Authorization:
   * upstreamCreds] → Upstream Proxy (may return 407) 3. If upstream returns 407, LittleProxy should
  ...
  void testScenario2_BothProxiesHandleAuth() {
    ...
    HttpResponse upstream407Response =
    ...
    boolean isUpstream407 =
        upstream407Response.status() == HttpResponseStatus.PROXY_AUTHENTICATION_REQUIRED;
    assertThat(isUpstream407).as("Should correctly identify upstream 407 response").isTrue();
   ...
   *
   * <p>Client → [Proxy-Authorization: upstreamCreds] → LittleProxy (no auth, preserves header) →
