...
/**
 * Represents an exclusion pattern for response headers.
 * This field logs all response headers EXCEPT those matching the given regex pattern.
 * Useful for excluding sensitive headers like Set-Cookie, etc.
 */
public class ExcludeResponseHeaderField implements LogField {
    ...
    /**
     * Creates a new exclusion-based response header field from a regex string with custom transformers.
     * @param excludeRegex the regex pattern string for headers to exclude
     * @param fieldNameTransformer function to transform header names to field names
     * @param valueTransformer function to transform header values
     */
